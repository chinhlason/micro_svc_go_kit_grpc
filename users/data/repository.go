...
func (r *Repository) Insert(ctx context.Context, username, password string) error {
	_, err := r.ss.ExecQuery(ctx, "INSERT INTO db2 (username, password) VALUES ($1, $2)", username, password)
	if err != nil {
		return err
	}
	return nil
...
func (r *Repository) Select(ctx context.Context, username string) (User, error) {
	rows, err := r.ss.QueryMultiRows(ctx, "SELECT * FROM db2 WHERE username = $1", username)
	if err != nil {
		return User{}, err
	}
	defer rows.Close()
